    ...
    <div class="row g-4">
        <div class="col-sm-12">
            <div class="bg-light h-100 rounded p-4">
                ...
                <form action="{{ url('update-owner/' . $owner->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="row">
                        <div class="col-lg-8 mb-3">
                            ...
                            <input type="text" class="form-control @error('name') is-invalid @enderror" name="name"
                                value="{{ old('name', $owner->name) }}" id="nama-kamar" autocomplete="off" required>
                            @error('name')
                    ...
                    <div class="col-lg-8 mb-3">
                        ...
                        <input type="email" class="form-control @error('email') is-invalid @enderror" name="email"
                            value="{{ old('email', $owner->email) }}" id="email" autocomplete="off" required>
                        @error('email')
                            <div class="invalid-feedback text-red">{{ $message }}</div>
                        @enderror
                    </div>
